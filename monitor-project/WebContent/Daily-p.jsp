...
<title>天文件：p(Multi-GNSS广播星历)</title>
		...
		<div id="page">
			<div id="content">
				...
				<div class="post">
					<h2 class="title">
						<a >成功站点 </a>
						<a>成功(${dPFileStationssuNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="dailyPFileStationssuccess" items="${dailyPFileStationssuccess }" varStatus="status">            
       						${dailyPFileStationssuccess } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
				...
				<div class="post">
					<h2 class="title">
						<a >失败站点 </a>
						<a>失败(${dPFileStationsfaNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="dailyPFileStationsfailed" items="${dailyPFileStationsfailed }" varStatus="status">            
       						${dailyPFileStationsfailed } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
			...
			<div id="sidebar">
				<ul>
					<li>
						<div id="search" >
							<form method="post" action="${pageContext.request.contextPath }/DailypFormServlet">
							  <div>
