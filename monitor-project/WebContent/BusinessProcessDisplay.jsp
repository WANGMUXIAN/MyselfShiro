...
<div id="wrapper">
	<div id="wrapper2">
		...
		<div id="page">
			<div id="content">
				<div class="post">
					<table border="1" width="580"  cellspacing="5" frame=void> 
	 					<tr align="center"> 
							<td bgcolor="#B7B7B7" width="155">业务进程</td> 
							<td bgcolor="#B7B7B7" width="75">状态</td> 
							<td bgcolor="#B7B7B7">上次运行结束时间</td> 
							<td bgcolor="#B7B7B7">下次计划启动时间</td> 
						</tr>
					 	<tr align="center">
							 <td bgcolor="#B7B7B7">超快轨道、ERP、对流层</td> 
							 <td>${PodUltLastStatus}</td> 
							 <td>${PodUltLastEndTime}</td>
							 <td>${PodUltNextBeginTime}</td>
						 </tr>
						 <tr align="center">
							 <td bgcolor="#B7B7B7">超快钟差</td> 
							 <td>${ClkUltLastStatus}</td> 
							 <td>${ClkUltLastEndTime}</td>
							 <td>${ClkUltNextBeginTime}</td>
						 </tr>
						 <tr align="center">
							 <td bgcolor="#B7B7B7">快速轨道、ERP、对流层</td> 
							 <td>${PodRpdLastStatus}</td> 
							 <td>${PodRpdLastEndTime}</td>
							 <td>${PodRpdNextBeginTime}</td>
						 </tr>
						 <tr align="center">
							 <td bgcolor="#B7B7B7">快速钟差</td> 
							 <td>${ClkRpdLastStatus}</td> 
							 <td>${ClkRpdLastEndTime}</td>
							 <td>${ClkRpdNextBeginTime}</td>
						 </tr>
						 <tr align="center">
							 <td bgcolor="#B7B7B7">快速电离层</td> 
							 <td>${IonRpdLastStatus}</td> 
							 <td>${IonRpdLastEndTime}</td>
							 <td>${IonRpdNextBeginTime}</td>
						 </tr>
						 <tr align="center">
							 <td bgcolor="#B7B7B7">最终轨道、ERP、对流层</td> 
							 <td>${PodFinLastStatus}</td> 
							 <td>${PodFinLastEndTime}</td>
							 <td>${PodFinNextBeginTime}</td>
						 </tr>
						 <tr align="center">
							 <td bgcolor="#B7B7B7">最终钟差</td> 
							 <td>${ClkFinLastStatus}</td> 
							 <td>${ClkFinLastEndTime}</td>
							 <td>${ClkFinNextBeginTime}</td>
						 </tr>
						 <tr align="center">
							 <td bgcolor="#B7B7B7">最终电离层</td> 
							 <td>${IonFinLastStatus}</td> 
							 <td>${IonFinLastEndTime}</td>
							 <td>${IonFinNextBeginTime}</td>
						 </tr>
				 </table>
