		...
		<div id="page">
			<div id="content">
				...
				<div class="post">
					<h2 class="title">
						<a >iGMAS站成功</a>
						<a>成功(${diGMASsuccessNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="dailyiGMASsuccess" items="${dailyiGMASsuccess }" varStatus="status">            
       						${dailyiGMASsuccess } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
				...
				<div class="post">
					<h2 class="title">
						<a >iGMAS站失败 </a>
						<a>失败(${diGMASfailedNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="dailyiGMASfailed" items="${dailyiGMASfailed }" varStatus="status">            
       						${dailyiGMASfailed } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
				...
				<div class="post">
					<h2 class="title">
						...
						<a>成功(${dIGS2successNum }个)</a>					
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="dailyIGS2success" items="${dailyIGS2success }" varStatus="status">            
       						${dailyIGS2success } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
       						<c:if test="${(status.index+1) % 8 == 0}">
        					<br/>
    						</c:if>        	
						</c:forEach>
				...
				<div class="post">
					<h2 class="title">
						...
						<a>失败(${dIGS2failedNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="dailyIGS2failed" items="${dailyIGS2failed }" varStatus="status">            
       						${dailyIGS2failed } &nbsp &nbsp
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
				...
				<div class="post">
					<h2 class="title">
						<a >MGEX站成功</a>
						<a>成功(${dMGEXIGS3successNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="dailyMGEXIGS3success" items="${dailyMGEXIGS3success }" varStatus="status">            
       						${dailyMGEXIGS3success } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
				...
				<div class="post">
					<h2 class="title">
						<a >MGEX站失败</a>
						<a>失败(${dMGEXIGS3failedNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="dailyMGEXIGS3failed" items="${dailyMGEXIGS3failed }" varStatus="status">            
       						${dailyMGEXIGS3failed } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
			...
			<div id="sidebar">
				<ul>
					<li>
						<div id="search" >
							<form method="post" action="${pageContext.request.contextPath }/DailydFormServlet">
							  <div>
