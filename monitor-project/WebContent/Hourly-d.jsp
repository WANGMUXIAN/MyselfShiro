	...
	<div id="wrapper2">
		...
		<div id="page">
			<div id="content">
				<div class="post">
					<h2 class="title">
						<a>小时文件：d（观测文件）下载情况</a>
					</h2>
					<p class="meta"><span class="posted"><font size="+2">(${year }/${month }/${date }  ${hour })</font> </span></p>	
					<div style="clear: both;">&nbsp;</div>
				</div>
				<div class="post">
					<h2 class="title">
						<a >iGMAS站成功</a>
						<a>成功(${hiGMASsuNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="hourlyiGMASsuccess" items="${hourlyiGMASsuccess }" varStatus="status">            
       						${hourlyiGMASsuccess } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
				...
				<div class="post">
					<h2 class="title">
						<a >iGMAS站失败 </a>
						<a>失败(${hiGMASfaNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="hourlyiGMASfailed" items="${hourlyiGMASfailed }" varStatus="status">            
       						${hourlyiGMASfailed } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
				...
				<div class="post">
					<h2 class="title">
						<a >IGS数据(RINEX 2.X)成功 </a>
						<a>成功(${hIGS2suNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="hourlyIGS2success" items="${hourlyIGS2success }" varStatus="status">            
       						${hourlyIGS2success } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
				...
				<div class="post">
					<h2 class="title">
						<a >IGS数据(RINEX 2.X)失败</a>
						<a>失败(${hIGS2faNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="hourlyIGS2failed" items="${hourlyIGS2failed }" varStatus="status">            
       						${hourlyIGS2failed } &nbsp &nbsp
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
				...
				<div class="post">
					<h2 class="title">
						<a >MGEX站成功</a>
						<a>成功(${hMGEXIGS3suNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="hourlyMGEXIGS3success" items="${hourlyMGEXIGS3success }" varStatus="status">            
       						${hourlyMGEXIGS3success } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
				...
				<div class="post">
					<h2 class="title">
						<a >MGEX站失败</a>
						<a>失败(${hMGEXIGS3faNum }个)</a>
					</h2>
					<div style="clear: both;">&nbsp;</div>
					<div class="entry">
						<c:forEach var="hourlyMGEXIGS3failed" items="${hourlyMGEXIGS3failed }" varStatus="status">            
       						${hourlyMGEXIGS3failed } &nbsp &nbsp  <!-- 每行输出四个站点，每个站点之间空两格 -->
       						<c:if test="${(status.index+1) % 8 == 0}">
        						<br/>
    						</c:if>        	
						</c:forEach>
			...
			<div id="sidebar">
				<ul>
					<li>
						<div id="search" >
							<form method="post" action="${pageContext.request.contextPath }/HourlydFormServlet">
							  <div>
									年：<input type="text" name="year" id="search-text" value="" /></br>
									月：<input type="text" name="month" id="search-text" value="" /></br>
									日：<input type="text" name="date" id="search-text" value="" /></br>
									时：<input type="text" name="hour" id="search-text" value="" />
