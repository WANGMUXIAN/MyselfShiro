...
//处理超快速erp文件
public class HandleNTUERPold {
	public static void HandleNtuErp(int IntHourFrom,int IntHourTo,long WeeksFrom,long WeeksTo,int DayNumWeekFrom,int DayNumWeekTo) throws IOException
	{
		String speed="NTU";
		...
		/*****   1   *****/
		/*****第一天时间处理***********/
		/*****这里的判断条件是“或”，所以这个判断分支既处理了同周不同天第一天的情况的情况和不同周第一天的情况****/
		...
		/********结束,最后一天处理********/
		if(WeeksFrom!=WeeksTo){
			//结束时间小时数位于0-2之间，并且周内天数为0，那么周内天数和周数都要减一
...
//				WriteProductFileName.write(StoreFileName1+"\r\n");
			...
			//结束时间小时数位于0-2之间，并且周内天数不为0，那么周内天数要减一，周数不用减一
			if(IntHourTo>=0&&IntHourTo<2&&DayNumWeekTo!=0)
			{
				//计算前一天的文件名
		...
		/********起始(开始结束)时间在同一周内，计算除了第一天和最后一天外的其他时间********/
		if(WeeksFrom==WeeksTo&&DayNumWeekFrom!=DayNumWeekTo)
		{
